...
export async function setSplatScene(name, view) {
  ...
  const splat = await SPLAT.PLYLoader.LoadAsync(
    bucket + "/" + name,
    scene,
    (progress) => (view.progressIndicator.value = progress * 100),
  );
  ...
  view.runningAnimation = (timestamp) => {
    ...
    if (!view.interacting) {
      ...
      const timeSinceClick =
        view.lastClick == undefined
          ? undefined
          : (new Date() - view.lastClick) / 1000.0;
      if (timeSinceClick > timeToSpin || timeSinceClick === undefined) {
        const speed =
          timeSinceClick === undefined
            ? maxSpinSpeed
            : Math.min(
                Math.max(timeSinceClick / accelTime - timeToSpin, 0.0),
                1.0,
              ) * maxSpinSpeed;
...
export async function setupCarousel(view, carousel) {
  ...
  async function onClickSplatThumb(splatName) {
    ...
    const itemsParent = carousel.getElementsByClassName(
      "splat-carousel-items",
    )[0];
    const items = [
      ...itemsParent.getElementsByClassName("splat-carousel-item"),
    ];
  ...
  for (var i = 0; i < files.length; ++i) {
    ...
    function setup(file) {
      ...
      card.addEventListener("mousedown", function () {
        startScroll = itemsParent.scrollLeft;
      });
  ...
  carousel
    .querySelector(".splat-carousel-button.left")
  ...
  carousel
    .querySelector(".splat-carousel-button.right")
    .addEventListener("mousedown", () => {
